...
interface IModuleRegistry {
    ...
    event OrgModuleSubscribed(uint48 indexed orgId, bytes32 indexed moduleKey);
    event OrgModuleUnsubscribed(uint48 indexed orgId, bytes32 indexed moduleKey);
     ...
     */
    function subscribeOrgToModule(uint48 orgId, bytes32 moduleKey) external;

    /**
     * @dev Huỷ đăng ký module cho Organization.
     */
    function unsubscribeOrgFromModule(uint48 orgId, bytes32 moduleKey) external;
     ...
     */
    function getOrgModules(uint48 orgId) external view returns (bytes32[] memory);
